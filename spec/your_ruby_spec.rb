...
RSpec.describe 'YourRuby' do
  ...
  describe '.calculate_completion_time' do
    ...
    it 'calculates when my prints will be ready if they take 2.5 hours to process and I order them at 17:30 on Thursday' do
      placed_time                = Time.parse('2020-02-27 17:30 UTC')
      calculated_completion_time = YourRuby.calculate_completion_time(placed_time, 2.5, opening_hours)
      expect(calculated_completion_time).to eq(Time.parse('2020-02-28 11:30 UTC'))
    end

    it 'calculates when my prints will be ready if they take 9 hours to process and I order them at 9:30 on Monday' do
      placed_time                = Time.parse('2020-02-24 9:30 UTC')
      calculated_completion_time = YourRuby.calculate_completion_time(placed_time, 9, opening_hours)
      expect(calculated_completion_time).to eq(Time.parse('2020-02-25 10:00 UTC'))
    end

    it 'calculates when my prints will be ready if they take 2 hours to process and I order them at 8:59 on Monday' do
      placed_time                = Time.parse('2020-02-24 8:59 UTC')
      calculated_completion_time = YourRuby.calculate_completion_time(placed_time, 2, opening_hours)
      expect(calculated_completion_time).to eq(Time.parse('2020-02-24 11:00 UTC'))
    end

    it 'calculates when my prints will be ready if they take 2 hours to process and I order them at 17:31 on Monday' do
      placed_time                = Time.parse('2020-02-24 17:31 UTC')
      calculated_completion_time = YourRuby.calculate_completion_time(placed_time, 2, opening_hours)
      expect(calculated_completion_time).to eq(Time.parse('2020-02-25 11:00 UTC'))
    end

    it 'calculates when my prints will be ready if they take 17 hours to process and I order them at 9:00 on Monday' do
      placed_time                = Time.parse('2020-02-24 9:00 UTC')
      calculated_completion_time = YourRuby.calculate_completion_time(placed_time, 17, opening_hours)
      expect(calculated_completion_time).to eq(Time.parse('2020-02-25 17:30 UTC'))
    end

    it 'calculates when my prints will be ready if they take 18 hours to process and I order them at 9:00 on Monday' do
      placed_time                = Time.parse('2020-02-24 9:00 UTC')
      calculated_completion_time = YourRuby.calculate_completion_time(placed_time, 18, opening_hours)
      expect(calculated_completion_time).to eq(Time.parse('2020-02-26 10:00 UTC'))
    end

    it 'calculates when my prints will be ready if they take 43 hours to process and I order them at 9:00 on Monday' do
      placed_time                = Time.parse('2020-02-24 9:00 UTC')
      calculated_completion_time = YourRuby.calculate_completion_time(placed_time, 43, opening_hours)
      expect(calculated_completion_time).to eq(Time.parse('2020-03-02 10:00 UTC'))
    end
  end
  ...
  describe '.duckduckwhy' do
    ...
    it 'gets the first result for "whytheluckystiff" from duckduckgo.com' do
      results = YourRuby.duckduckwhy('whytheluckystiff wiki', 1)
      expect(results).not_to be_nil
